...
import { Mail, Eye, EyeOff, ArrowRight, User, Smartphone } from "lucide-react";
import { motion } from "framer-motion"; // eslint-disable-line no-unused-vars
import SectionTitle from "../components/common/SectionTitle";
import axios from "axios"; // Import this
import { toast } from "react-toastify";
// Using a different model image for variety (e.g., the Men's collection or a different pose)
const registerModelImage = "/man.avif";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Usually, new registrations are 'user' role by default
        navigate("/");
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-[#FCFBFA] flex items-center justify-center p-4 sm:p-6 md:p-10">
      ...
      >
        ...
        <div className="w-full md:w-1/2 p-8 lg:p-16 flex flex-col justify-center">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button className="cursor-pointer flex-1 py-3.5 border border-[#D4CDCA]/40 rounded-xl hover:bg-[#FCFBFA] hover:border-[#E0655F]/30 transition-all flex items-center justify-center gap-3 text-[10px] font-bold uppercase tracking-widest text-[#1A1A1A]">
        ...
        <div className="hidden md:block w-1/2 relative overflow-hidden group">
          ...
          <div className="absolute top-10 right-10 w-16 h-16 bg-white/5 backdrop-blur-xl rounded-full flex items-center justify-center border border-white/10">
